module SecEdgar
  class ReportCreator < Base
    ...
    def perform(report_id, html_file)
      ...
      xlsx_file = "/tmp/#{report.company.symbol} #{report.filed_at.year} #{report.company.name}_Financials.xlsx"

      pdf = WickedPdf.new.pdf_from_string html
      xls = open(URI.join(SEC_ARCHIVES_URL, report.excel_path).to_s).read

      report.add_dropbox_file :pdf,   pdf__file, pdf, true
      report.add_dropbox_file :excel, xlsx_file, xls, true
